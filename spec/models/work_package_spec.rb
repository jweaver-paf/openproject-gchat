...
describe WorkPackage, with_settings: { "host_name" => "test.openproject.com", "protocol" => "https" } do
  ...
  describe 'update' do
    ...
    before do
      work_package # create work package before gchat is enabled so we only test updates

      allow(::OpenProject::gchat).to receive(:default_webhook_url).and_return("https://foo.bar.com/webhook/42")
    end
    ...
    before do
      expect(OpenProject::gchat::Notifier).to receive(:say) do |opts|
