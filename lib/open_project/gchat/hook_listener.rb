require 'open_project/gchat/notifier'

class OpenProject::gchat::HookListener < OpenProject::Hook::Listener
  def url_helpers
  ...
  def work_package_after_create(context={})
    ...
    OpenProject::gchat::Notifier.say text: message, attachments: [attachment], webhook_url: webhook_url
  end
  ...
  def project_webhook_url(project)
    url = project.custom_values
      .joins(:custom_field)
      .where(custom_fields: { name: OpenProject::gchat::webhook_url_label })
      .pluck(:value)
      .first

    url ||= project_webhook_url(project.parent) if project.parent.present?
    url ||= OpenProject::gchat.default_webhook_url
  ...
  def extract_usernames(text = '')
    # gchat usernames may only contain lowercase letters, numbers,
